...
import { verifyEmailAPI, resendOtpAPI } from "@/actions/auth";
...
function OTPVerification({ email }: OTPVerificationProps) {
  ...
  const handleVerifyEmail = async () => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.status === "success") {
        ...
        const errorMessage = response.data?.message || response.data?.error || 'Verification failed';
  ...
  const handleResendCode = async () => {
    try {
      ...
      if (response.status === 200 && response.data.status === "success") {
        ...
        const errorMessage = response.data?.message || response.data?.error || 'Failed to resend code';
        toast.error(errorMessage);
      }
    } catch (error: any) {
  ...
  return (
    <div className="p-8 md:p-10 w-full lg:w-[75%] flex flex-col">
      <div className="flex lg:items-start flex-col">
        ...
        <p className="text-sm font-semibold text-primary-color mb-6">{email}</p>
      </div>
